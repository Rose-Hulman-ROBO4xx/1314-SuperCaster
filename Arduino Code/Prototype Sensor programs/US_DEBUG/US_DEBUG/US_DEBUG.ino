...
void setup() {
  ...
  //Sensors:  
  int i=0;
  digitalWrite(US_enables[0],HIGH);
  delay(20);
...
int readUS(){
  ...
  for(i=0;i<NUM_US;i++){
    if(i==US_grpCount){
        US_read[US_grpCount] = analogRead(US_pins[US_grpCount]);
        if(US_grpCount > 6){
            US_grpCount = 0;
        }
        US_grpCount++; 
        digitalWrite(US_enables[US_grpCount],HIGH);
        delay(2);
        digitalWrite(US_enables[US_grpCount],LOW);
    }
